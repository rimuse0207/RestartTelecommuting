...
const WeekAfterOTContainer = () => {
    ...
    return (
        <div>
            {loginChecked ? (
                <div>
                    <HambergerMenu titles={BeforeORAfter ? "OT사전 신청" :"OT사후 신청" }subtitles=""></HambergerMenu>
                    <div>
                        <div className="WeekOTMenubar_big_div">
                            <ul>
                                <li style={BeforeORAfter ? {backgroundColor: "#2da8e5"}:{}} onClick={()=>setBeforeORAfter(true)}>사전 OT</li>
                                <li style={BeforeORAfter ? {}:{backgroundColor: "#f7c80e"}} onClick={()=>setBeforeORAfter(false)}>사후 OT</li>
                            </ul>
                        </div>
                    </div>
                    <div>
                        {BeforeORAfter ? <WeekBeforeOTWorkSpace startDate={startDate} endDate={endDate} setStartDate={setStartDate} setEndDate={setEndDate}></WeekBeforeOTWorkSpace> :<WeekAfterOTWorkSpace startDate={startDate} endDate={endDate} setStartDate={setStartDate} setEndDate={setEndDate}></WeekAfterOTWorkSpace>}
