...
import { Printer_Button_Container_Div_Box } from '../PersonDBClickModal';

const MonthOtPersonShow = () => {
    const User_Info_State = useSelector((state: RootState) => state.PersonalInfo.infomation);
    ...
    const HandleClicksPrinterPerson = () => {
        window.open(
            `/OtPersonPrinter/${DecryptKey(User_Info_State.name)}/${moment(getMoment).format('YYYY')}/${moment(getMoment).format('MM')}`,
            'popup',
    ...
    const getDataForMe = async () => {
        const User_Name = DecryptKey(User_Info_State.name);
        const User_Id = DecryptKey(User_Info_State.email);
        try {
    ...
    return (
        <div>
            <HambergerMenu titles={'월별 OT 조회'} subtitles={''}></HambergerMenu>
            <div className="control">
                <button
                    onClick={() => {
                        setgetMoment(getMoment.clone().subtract(1, 'month'));
                    }}
                >
                    {'<<<'}
                </button>
                ...
                <button
                    onClick={() => {
                        setgetMoment(getMoment.clone().add(1, 'month'));
                    }}
                >
                    {'>>>'}
                </button>
            ...
            {dataloading ? (
                <div className="PersonOTSelect_div_box" style={{ padding: '10px' }}>
                    <table>
                        <thead>
                        ...
                        <tbody>
                            ...
                            {Array(
                                new Date(parseInt(moment(getMoment).format('YYYY')), parseInt(moment(getMoment).format('MM')), 0).getDate()
                            )
                                .fill(0)
                                .map((list, i) => {
                                    var counts = 0;
                                    var Mapdate = `${moment(getMoment).format('YYYY-MM')}-${i + 1 > 9 ? i + 1 : '0' + (i + 1)}`;
                                    return (
                                        <tr
                                            style={
...
export default MonthOtPersonShow;
