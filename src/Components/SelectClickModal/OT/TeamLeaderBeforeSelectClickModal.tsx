...
import { getTeamLeaderBEFOREOTdataThunk } from '../../../models/TeamLeader_Thunk_models/TeamLeaderBeforeOTData';
...
const TeamLeaderBeforeSelectClickModal = ({ clicksTitle, clicksData, modalClose }: TeleSelectClickModalProps) => {
    ...
    const handleDataClick = async () => {
        try {
            const BeforeOTAcceptData = await axios.post(
                `${process.env.REACT_APP_API_URL}/TeamSelectOT_app_server/BeforeOTdataTeamLeaderAccept`,
                {
                    clicksData,
                }
            );
            if (BeforeOTAcceptData.data.dataSuccess) {
    ...
    const handleBeforeOTCancel = async () => {
        try {
            const BeforeOTAcceptData = await axios.post(`${process.env.REACT_APP_API_URL}/TeamSelectOT_app_server/BeforeOTDataCancel`, {
                clickedOTData: clicksData,
            });
            if (BeforeOTAcceptData.data.dataCheck) {
                dispatch(getTeamLeaderBEFOREOTdataThunk(moment(clicksData.date_mon).format('YYYY-MM'), InfomationState));
                toast.show({
                    title: '팀장 승인 취소 완료.',
                    content: `${clicksData.name} 팀원의 사전OT 부문에 승인 취소하였습니다.`,
                    duration: 6000,
                    DataSuccess: true,
                });
            } else {
                toast.show({
                    title: '팀장 승인 취소 실패.',
                    content: `사전OT 부문에 승인 취소에 실패 하였습니다.(IT팀에 문의바람.)`,
                    duration: 6000,
                    DataSuccess: false,
                });
            }
    ...
    const getSomeData = async (clicksData: any) => {
        ...
        try {
            const getSomeDatas = await axios.post(`${process.env.REACT_APP_API_URL}/TeamSelectOT_app_server/BeforeOTDataSelectModal`, {
            ...
            <div>
                {clicksData.leadercheck === 0 ? (
                    ...
                    <div style={{ textAlign: 'end', marginTop: '30px' }}>
                        ...
                        <div className="AcceptOkayDiv" onClick={() => modalClose()} style={{ display: 'block' }}>
                            승인완료.
                        </div>
                    </div>
                )}
...
export default TeamLeaderBeforeSelectClickModal;
